...
export default function BackgroundImage({ src, gradientDirClass, middleColourPercentClass, fromClass="from-bgcol", toClass = "to-bgcol/30", objectPositionClass, priority=false, sizes }: { src: string, gradientDirClass: string, middleColourPercentClass: string, fromClass?: string, toClass?: string, objectPositionClass?: string, priority?: boolean, sizes: string }) {
    return (
        <div className="w-full h-full absolute top-0 grain-img">
            <Image src={src} fill className={`object-cover bg-accentcol ${objectPositionClass ? objectPositionClass : ""} saturate-150 contrast-125 brightness-110`} alt="" priority={priority} sizes={sizes} />
